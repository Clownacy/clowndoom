  ...
  fixed_t       y )
{
	...
	angle = FixedDiv(dy,dx);
	angle = (tantoangle[ CC_MIN(FRACUNIT,angle)>>DBITS ]+ANG90) >> ANGLETOFINESHIFT;
...
#define DISTMAP         (2*LIGHTINGRESOLUTIONSCALE)
...
void R_ExecuteSetViewSize (void)
{
	...
	/* psprite scales */
	pspritescale = FRACUNIT*viewwidth/SCREENWIDTH*SCREENHEIGHT/ORIGINAL_SCREEN_HEIGHT;
	pspriteiscale = FRACUNIT*SCREENWIDTH/viewwidth*ORIGINAL_SCREEN_HEIGHT/SCREENHEIGHT;
...
void R_SetupFrame (player_t* player)
{
	...
	{
		fixedcolormap =
			colormaps[player->fixedcolormap];
