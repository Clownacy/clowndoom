...
#define MAXVISPLANES    1024 /* Previously 128, raised because No Rest for the Living MAP03 goes over the limit when rendering in HD. */
...
int                     spanstart[MAXIMUM_SCREENHEIGHT];
int                     spanstop[MAXIMUM_SCREENHEIGHT];
...
fixed_t                 cachedxstep[MAXIMUM_SCREENHEIGHT];
fixed_t                 cachedystep[MAXIMUM_SCREENHEIGHT];
...
void R_DrawPlanes (void)
{
	...
	if (ds_p - drawsegs > MAXDRAWSEGS)
		I_Error ("R_DrawPlanes: drawsegs overflow (%li)",
				 (unsigned long)(ds_p - drawsegs));

	if (lastvisplane - visplanes > MAXVISPLANES)
		I_Error ("R_DrawPlanes: visplane overflow (%li)",
				 (unsigned long)(lastvisplane - visplanes));

	if (lastopening - openings > MAXOPENINGS)
		I_Error ("R_DrawPlanes: opening overflow (%li)",
				 (unsigned long)(lastopening - openings));
#endif
