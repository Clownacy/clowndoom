...
void M_DrawLoad(void)
{
	...
	V_DrawPatchDirect (X_CENTRE(72),Y_CENTRE(28),0,(patch_t*)W_CacheLumpName("M_LOADG",PU_CACHE));
	for (i = 0;i < load_end; i++)
	{
...
void M_DrawSaveLoadBorder(int x,int y)
{
	...
	V_DrawPatchDirect (x-8*SCREEN_MUL,y+7*SCREEN_MUL,0,(patch_t*)W_CacheLumpName("M_LSLEFT",PU_CACHE));

	for (i = 0;i < 24;i++)
	{
		V_DrawPatchDirect (x,y+7*SCREEN_MUL,0,(patch_t*)W_CacheLumpName("M_LSCNTR",PU_CACHE));
		x += 8*SCREEN_MUL;
	}

	V_DrawPatchDirect (x,y+7*SCREEN_MUL,0,(patch_t*)W_CacheLumpName("M_LSRGHT",PU_CACHE));
}
...
void M_DrawSave(void)
{
	...
	V_DrawPatchDirect (X_CENTRE(72),Y_CENTRE(28),0,(patch_t*)W_CacheLumpName("M_SAVEG",PU_CACHE));
	for (i = 0;i < load_end; i++)
	{
		M_DrawSaveLoadBorder(LoadDef.x,LoadDef.y+LINEHEIGHT*i);
		M_WriteText(LoadDef.x,LoadDef.y+LINEHEIGHT*i,savegamestrings[i]);
...
void M_DrawReadThis1(void)
{
	...
	{
	  case commercial:
		V_DrawPatchDirect (X_CENTRE(0),Y_CENTRE(0),0,(patch_t*)W_CacheLumpName("HELP",PU_CACHE));
		break;
	  case shareware:
	  case registered:
	  case retail:
		V_DrawPatchDirect (X_CENTRE(0),Y_CENTRE(0),0,(patch_t*)W_CacheLumpName("HELP1",PU_CACHE));
		break;
	  default:
		break;
	}
...
void M_DrawReadThis2(void)
{
	...
	switch ( gamemode )
	{
	  ...
	  case commercial:
		...
		V_DrawPatchDirect (X_CENTRE(0),Y_CENTRE(0),0,(patch_t*)W_CacheLumpName("CREDIT",PU_CACHE));
		break;
	  case shareware:
	  case registered:
		V_DrawPatchDirect (X_CENTRE(0),Y_CENTRE(0),0,(patch_t*)W_CacheLumpName("HELP2",PU_CACHE));
		break;
	  default:
		break;
	}
...
void M_DrawSound(void)
{
	V_DrawPatchDirect (X_CENTRE(60),Y_CENTRE(38),0,(patch_t*)W_CacheLumpName("M_SVOL",PU_CACHE));
...
void M_DrawMainMenu(void)
{
	V_DrawPatchDirect (X_CENTRE(94),Y_CENTRE(2),0,(patch_t*)W_CacheLumpName("M_DOOM",PU_CACHE));
}
...
void M_DrawNewGame(void)
{
	V_DrawPatchDirect (X_CENTRE(96),Y_CENTRE(14),0,(patch_t*)W_CacheLumpName("M_NEWG",PU_CACHE));
	V_DrawPatchDirect (X_CENTRE(54),Y_CENTRE(38),0,(patch_t*)W_CacheLumpName("M_SKILL",PU_CACHE));
}
...
void M_DrawEpisode(void)
{
	V_DrawPatchDirect (X_CENTRE(54),Y_CENTRE(38),0,(patch_t*)W_CacheLumpName("M_EPISOD",PU_CACHE));
}
...
void M_DrawOptions(void)
{
	V_DrawPatchDirect (X_CENTRE(108),Y_CENTRE(15),0,(patch_t*)W_CacheLumpName("M_OPTTTL",PU_CACHE));

	V_DrawPatchDirect (OptionsDef.x + 175*SCREEN_MUL,OptionsDef.y+LINEHEIGHT*detail,0,
					   (patch_t*)W_CacheLumpName(detailNames[detailLevel],PU_CACHE));

	V_DrawPatchDirect (OptionsDef.x + 120 * SCREEN_MUL,OptionsDef.y+LINEHEIGHT*messages,0,
  ...
  int   thermDot )
{
	...
	V_DrawPatchDirect ((x+8*SCREEN_MUL) + thermDot*8*SCREEN_MUL,y,
...
( menu_t*       menu,
  int           item )
{
	V_DrawPatchDirect (menu->x - 10*SCREEN_MUL,        menu->y+item*LINEHEIGHT - 1*SCREEN_MUL, 0,
...
void M_Drawer (void)
{
	...
	/* DRAW SKULL */
	V_DrawPatchDirect(x + SKULLXOFF,currentMenu->y - 5*SCREEN_MUL + itemOn*LINEHEIGHT, 0,
